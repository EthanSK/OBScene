...
- **Trigger actions.** Per profile, mix and match any of:
  ...
  - One-shot **Refresh macOS capture source** using OBS's native action
...
- **Opt-in macOS capture refresh.** Use **Refresh macOS Capture Source** in the menu-bar dropdown for a manual attempt, or enable **Refresh macOS capture source** in an individual profile's Trigger Actions. Both paths use OBS's native action only and never change source settings; global wake/display/recording recovery can remain disabled.
