...
Dock into your battlestation and OBScene takes care of the boring bits: it switches your scene collection, profile, and active scene, then (optionally) starts recording or streaming. Unplug and it can stop everything just as cleanly. All hands-free, all local.
...
- **Configurable threshold** — decide how many external displays must be connected before OBScene fires (e.g. "when 2 external displays are connected").
- **Scene / profile / scene collection switching** — pick exactly what OBS should switch to, or leave any of them untouched.
- **Start recording and/or streaming** on trigger — optional, independent toggles.
- **Stop on unplug** — optionally stop recording and/or streaming the moment displays drop below the threshold. Perfect for "record while I'm docked, stop when I pack up."
- **Configurable delay** — a debounce window (default 15s) before the start actions fire, so OBS has time to settle after a display change.
...
6. **Pick your trigger actions**
   - Toggle **Start Recording** and/or **Start Streaming**.
...
7. **Set the display threshold and delay**
   ...
   - Set the debounce delay (default 15 seconds) before start actions fire.
...
- **Going down** (count crosses threshold downward): immediately fires the unplug trigger, which sends `StopRecord` / `StopStream` to OBS for whichever stop-on-unplug options you enabled.
...
| **External display threshold** | Number of external displays required to fire the trigger. | `2` |
| **Trigger delay** | Seconds to wait after the threshold is crossed before firing start actions. | `15` |
